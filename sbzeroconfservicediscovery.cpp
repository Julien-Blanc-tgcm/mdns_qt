...
void SBZeroConfServiceDiscovery::parseData_()
{
	if (auto sender = dynamic_cast<QUdpSocket*>(QObject::sender()))
	{
		while (sender->hasPendingDatagrams())
		{
			...
			for (int i = 0; i < packet.answers_count() + packet.additional_count(); ++i)
			{
			...
			if (error)
			{
				qDebug() << "Not ok :" << hostAddress << hostPort << record.hostname();
				continue;
			}
			...
			{
				qDebug() << "Not everything ok:" << hostAddress << hostPort << record.hostname() << addressOk << portOk
				         << nameOk;
			}
		}
	}
